...
*/
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis DAMA baik hati`)
				nul = body.slice(7)
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 DAMA baik hati`)
				isall = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DULU YAKAN')
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 DAMA baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DULU YAKAN')
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo DAMA & Gans`)
				reply(nad.wait())
		...
		case 'halloweentext':
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub DAMA & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}glitchtext DAMA & Gans`)
					reply(nad.wait())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo DAMA', text, {quoted: mek})
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata DAMA`
...
case 'neontext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}neontext DAMA`)
					neon = body.slice(10)
					reply('「❗」WAIT GANS')
...
*/		 
				...
				if (!isRegistered) return reply(nad.noregis())
					if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Contoh : ${prefix}blackpink DAMA`)
					pink = body.slice(11)
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner DAMA`)
                      if (isOwner) {
                      const one = 999999999
...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}hartatahta DAMA`)
har = body.slice(12)
